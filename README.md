...
PlantUMLの図をローカル環境で生成する場合は、Java、PlantUML、Graphvizが必要です。Dockerを使う場合、これらの依存関係はDockerイメージに含まれます。
...
TeXの環境構築が困難な場合は、Dockerでビルドできます。このリポジトリのDockerfileは、Re:VIEW 5.9のイメージへPlantUMLの描画に必要なGraphvizを追加します。

Dockerが動作する環境で、リポジトリのルートから次のスクリプトを実行してください。Dockerイメージの作成、依存パッケージの導入、PDF生成までを実行します。

```
$ ./build-in-docker.sh
```

生成したPDFは `articles/ReVIEW-Template.pdf` に出力されます。
...
```
$ docker compose run --build -e REVIEW_CONFIG_FILE=config.yml --rm review
```
